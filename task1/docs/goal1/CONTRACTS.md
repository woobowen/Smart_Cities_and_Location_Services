...
65/5/30/400/35/5 均来自基础 Notebook Cell1，分别为长度/点数/秒/意图米/度/意图米，标 STARTER_REFERENCE；本轮参考运行固定这些值，但未据此选定最终参数或宣称最优。PPT24 的95m是25 km/h骑行、12秒采样示例，不能与400混为同一配置。
...
方向审核仅对报告的两个归一化角差使用圆周输入尺度 `64 * binary64_epsilon * 360°`，不是对35°决策阈值加余量。首个真实运行揭示直接差值与模360公式在小角差处的减法消去：两处差异为约5.68e-14°，删留决策一致。修复审核器的派生数值比较后，candidate、索引、undefined理由和删除集合仍严格核对，DP数值保证不变。失败运行原回执保留，最终运行使用新代码/新ID。
...
| 片段与过滤 | 切分边右归，过滤理由逐段，段内长度 | 条、点、平面单位/描述性/全部片段 | 当前长度是获准局部模型中的米；真实地面长度仍无来源证明 | 结构保留与损失；geometry.split_trajectory/filter_segments |
